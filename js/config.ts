const getDebugFlag = (): boolean => {
  try {
    return Boolean((globalThis as any).__CET46_DEBUG__);
  } catch (_error) {
    return false;
  }
};
...
export const CONFIG = {
  ...
  CACHE_NAME: 'cet46-v2.0.2',
  AUDIO_CACHE_NAME: 'cet46-audio-cache-v1',
  ...
  STORAGE_KEYS: {
    ...
    SYNC_BASE: 'cet46_sync_base',
    SHORTCUT_GUIDE_SHOWN: 'cet46_shortcut_guide_shown',
    TARGET_RETENTION: 'cet46_target_retention',
  ...
  TOTAL_VOCAB_COUNT: 6662,
  SYNC_DEBOUNCE_MS: 1000,

  LRU_LIMIT_PROGRESS: 7000,
  ...
  CONSTANTS: {
    ...
    // 单词等级上限：学习/复习路径必须使用同一上限，否则 mastered 状态永远无法达成
    LEVEL_CAP: 10,
    LEVEL_MASTERED: 10,
    // 复习间隔上限（毫秒）：365 天，避免一次答对后下次复习间隔过长
